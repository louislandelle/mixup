...
def train(device, model, optimizer, criterion, lam_vec, trainloader, *, 
          n_epochs=None, n_batches=None,
          save_raw=False, save_state_dicts="", save_tensorboard="", results_fname="0",
    ...
    """
    print_ = get_print(save_raw, fname=results_fname)
...
def test(device, model, testloader, *, results_fname="0", save_raw=False):
    print_ = get_print(save_raw, fname=results_fname)
...
def collect_results_nmix(trainloader, testloader, lambdas, *,
    model_getter=resnet18, n_epochs=10, n_batches=None, half=False,
    no_mixup=False, save_raw=False, save_state_dicts=False, save_tensorboard=False, results_fname="0",
    ...
    for lambda_ in lambdas:
        ...
        train(device, model, optimizer, criterion, lam_rs_tensor, trainloader,
        ...
        test_accs[str(lambda_)] = test(device, model, testloader, results_fname=results_fname, save_raw=save_raw)
